...
# Hardcoded timeline from src/components/TimelineSection.tsx (fallback)
timeline = [
...
# Update dates (shift by 7 days starting from the registration end)
updated_timeline = []
for stage in timeline:
    # Only shift if it's after the current pendaftaran end date (2027-02-07)
    # Actually, the user says "di majukkan 7 hari lagi dari pendaftaran hingga awarding"
    # "Pendaftaran" end is 2027-02-07. If we add 7 days, it becomes 2027-02-14.
    
    new_stage = stage.copy()
    
    # We shift 'date' if it's 2027-02-07 or later.
    # We shift 'startDate' if it's 2027-02-08 or later (since pendaftaran is usually the first).
...
print(json.dumps(updated_timeline, indent=2))
